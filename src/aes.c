                        ...
                        const uint8_t * schedule,uint8_t *ciphertext)
{
    ...
    {
        fprintf(stderr, "Error: Input plaintext_len must be multiple of %d\n", AES_BLOCK_SIZE);
        return;
    }

    uint8x16_t *subkeys = (uint8x16_t *)schedule;
    const uint32_t rounds = AES128_ROUNDS;

    while (plaintext_len >= AES_BLOCK_SIZE)
    {
        ...
        block = veorq_u8(block,subkeys[10]);
